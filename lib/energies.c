...
/* UNF is a global variable for bookkeeping the dielectric boundary
...
{  FILE *fp;
   ...
   /* Prepare run delphi in a different directory */
   ...
   */
   ...
   for (i=0; i<prot.n_res; i++) {
      ...
      /* By default, the first charged conformer is picked, which is necessary for the boundary correction.
         For some ions, the first conformer is forced to be picked.
         This is added for Yifan's surface ion binding calculations so that ion-protein interactions are
         calculated assuming the rest of the ions not occupied. A problem with this is that the ion-ion
         interactions can only be calculated in the multi-conf delphi runs with the wrong boundary condition.
         Yifan
      */
      /* Which version has the "wrong boundary conditions"?
         Cat 2014/4
      */
...
{  int i, ic, j, k, counter;
		...
		/*final delta rxn energy of the conformer*/
			...
			*/
			...
			if (!strcmp(env.rxn_method,"surface") || !strcmp(env.rxn_method,"ntsurface")) {
				...
				for (i=0; i<counter; i++)
                                {
				   fprintf(fp, "%30s%8.3f%8.3f%8.3f\n", head_res[i], unf_res[i].x, unf_res[i].y, unf_res[i].z);
			        }
				fclose(fp);
   ...
   for (i=0; i<prot.n_res; i++)
   {
      for (j=1; j<prot.res[i].n_conf; j++)
      {
         ...
         sscanf(sbuff, "%d %s %f %f", &prot.res[i].conf[j].iConf, uniqID, &prot.res[i].conf[j].tmp_pw_ele, &prot.res[i].conf[j].tmp_pw_vdw);
   ...
   for (i=0; i<prot.n_res; i++) {
      fgets(sbuff, sizeof(sbuff), fp);
      sscanf(sbuff, "%d %s %f %f", &counter, uniqID, &prot.res[i].conf[0].tmp_pw_ele, &prot.res[i].conf[0].tmp_pw_vdw);
   }
   rxnmulti[0] = '\n';
   while (fgets(sbuff, sizeof(sbuff), fp))
   {
      if (!strncmp("RXNMULTI", sbuff, 8)) {
         strcpy(rxnmulti, sbuff); //which index of rxnmulti gets sbuf??
         break;
      }
   }
   fclose(fp);
   ...
   fprintf(fp, "ELE_BKBN  %10.3f\n", ele0);
   fprintf(fp, "TORSION   %10.3f\n", torsion);
   fprintf(fp, rxnmulti);//ERROR:1819:4: warning: format not a string literal and no format arguments [-Wformat-security]
