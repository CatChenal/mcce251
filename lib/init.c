...
#include <math.h>
#include "mcce.h"
/*    v.2.5.1 -> should be v.2.5.2 bc it does an additional titration
      2014_Apr: Amended by catalys: Added check on third type of titration when ch is given instead of ph or eh;
                -> ch becomes 'x' in the enveloppe with associated parameters EX0 and EXD.
                Amended mcce.h ENV with titr_ex0 and titr_exd;
                Also amended monte.c and monte2.c accordingly.
*/
ENV env;
...
{   FILE *fp;
    ...
    // Call to get_env: collects all the parameters in struc
     ...
     * 2. tpl file (usually extra.tpl)
     Previously:  * 3. default value from env.epsilon_prot
     */
    kscale = 1.0; /*Previously: kscale = env.epsilon_prot; -> scaling factor based on dielectric constant */
    if (env.scale_vdw0 < 0) {
...
{   FILE *fp;
    ...
    env.hv_relax_shake_tol =  1e-4;  /* constraint distance */
    ...
    strcpy(env.bcfl, "sdh");     /*specify the type of boundry condition used -"Single Debye-Hukel" boundary condition*/
...
/// ..............  titra type:
            ...
            str1 = strtok(sbuff, " ");
            if (str1[0] == 'p' || str1[0] == 'P') {
		env.titr_type = 'p';
	    }
            else if (str1[0] == 'e' || str1[0] == 'E') {
                env.titr_type = 'e';
            }
            else env.titr_type ='x';  // for Extra titration
	}
        ...
        else if (strstr(sbuff, "(TITR_EX0)")) { // for Extra titration
            env.titr_ex0 = atof(strtok(sbuff, " "));
        }
        else if (strstr(sbuff, "(TITR_EXD)")) { // for Extra titration
            env.titr_exd = atof(strtok(sbuff, " "));
        }
...
// ....... run.trace:
        ...
        /* this block prints headers for all the columns */
        ...
        while (fgets(trbuff, sizeof(trbuff), fp)) {
			if (strstr(trbuff, "(ADDING_CONF)")) {
				...
				fprintf(tr, "%31s%31s\t%31s%31f\n", "(TITR_PHD)", strtok(trbuff, " "), "env.titr_phd", env.titr_phd);
                        ...
                        else if (strstr(trbuff, "(TITR_EX0)")) {
                                fprintf(tr, "%31s%31s\t%31s%31f\n", "(TITR_EX0)", strtok(trbuff, " "), "env.titr_ex0", env.titr_ex0);
                        }
                        else if (strstr(trbuff, "(TITR_EXD)")) {
                                fprintf(tr, "%31s%31s\t%31s%31f\n", "(TITR_EXD)", strtok(trbuff, " "), "env.titr_exd", env.titr_exd);
                        }
