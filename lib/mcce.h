...
typedef struct {
	int Nelectrons;
	int Nprotons;
//	int Nconfermers; => error in monte2.c [monte2 and load_states] bc member ref'd w/o typo:error: ISTATES - has no member named - Nconformers
	int Nconformers;
	float Cfactor;
	float totalOCC;
} ISTATES;
...
typedef struct {
    ...
    char   altLoc;
    char   confID[4];
    int    iConf;        /* Used to identify a conformer from mcce pdb, also the index number to the head list */
...
typedef struct {
    ...
    float titr_eh0;
    float titr_ehd;
    float titr_ex0; //@@@ Amended for 'extra energy' titration
    float titr_exd;
