...
#include <time.h>
#include <math.h>
// Next 2 lines: to add in order to catch -nan (NotAnumber) errors? (2/23/15)
//#include <fenv.h>
//feenableexcept(FE_DIVBYZERO | FE_INVALID | FE_OVERFLOW);
// See http://stackoverflow.com/questions/14990565/catch-nan-and-inf-in-windows-c?rq=1
 ...
 * entropy effect
 *
 * Normal pwise is garanteed to be smaller than 2000. When it is is bigger than 5000, it is
 ...
 * counter in conflist
 *
 * Amended Apr 2014 to implement an 'extra energy' titration as per Xuyu in /home/xzhu/mcce_version/Old/mcce2.4.4_extra/lib
 */
//..............................................................................................
...
typedef struct  {
    ...
    int on;
    int *conf;  /* indices of free confs */
...
RESIDUE  *free_res, *fixed_res, *all_res, *mfe_res; /* mfe_res: all the mfe residues */
...
//............................................................................
float    ph, eh, extra; /* extra: energy amount added to self0 :: 'extra energy titration'; defined but not used? */
//............................................................................
...
int monte()
{
	...
	ph = env.titr_ph0;
        eh = env.titr_eh0;
        extra = env.titr_ex0;  //@@@ Amended for extra titration ...................................................
	...
	else { extra = env.titr_ex0 + i*env.titr_exd; } //@@@ Amended for extra titration

        fprintf(fp, "Titration (%dth) at pH = %6.2f,  Eh = %6.2f, Extra = %6.2f:\n", i+1, ph, eh, extra); fflush(fp);
	...
	if (N_smp) MC(N_smp);
        ...
        fprintf(fp, "Doing equilibration ... \n"); fflush(fp);
        if (N_smp) MC(N_smp);
        fprintf(fp, "Done\n");
        ...
        if (env.monte_tsx) {
           while (S_max > 0.5) {
             E_base = get_base();
             if (enumerate(i) == -1) { /* Non-ANALYTICAL SOLUTION */
                counter = 0;
        ...
        for (j=0; j<conflist.n_conf; j++) {
            ...
            fprintf(fp, "%s   %c %8.3f", conflist.conf[j].uniqID, conflist.conf[j].on, conflist.conf[j].E_self0);
    ...
    if (env.titr_type == 'p') {
        ...
        fprintf(fp, " eh           ");
        for (i=0; i<env.titr_steps; i++) { fprintf(fp, " %5.1f", env.titr_eh0+i*env.titr_ehd); }
    }
    else { //@@@ Amended for extra titration
        fprintf(fp, " extra        ");
        for (i=0; i<env.titr_steps; i++) { fprintf(fp, " %5.1f", env.titr_ex0+i*env.titr_exd); }
    }
    fprintf(fp, "\n");
    ...
    /* free MC stat */
    for (i=0; i<env.monte_runs; i++) { free(MC_occ[i]); }
    free(MC_occ);
    free(sigma);
    /* free conflist */
    free(conflist.conf);
    ...
    /* free pairwise */
    for (i=0; i<conflist.n_conf; i++) { free(pairwise[i]); }
    free(pairwise);
    /* free residues */
    for (i=0; i<n_free; i++) { free(free_res[i].conf); }
    free(free_res);
...
float get_base()
{
    ...
    for (kc=0; kc<conflist.n_conf; kc++)
    {
        ...
        // "extra" from Extra titration added to all DM conformers ->H2O, ions, ligands
        if (!strncmp(conflist.conf[kc].uniqID+3, "DM", 2)) conflist.conf[kc].E_self0 += extra;
    }
...
void MC(int n)
{
    ...
    for (i=0; i<cycles; i++)
    {
        ...
        while (iters)
        {
             ...
             */
            if (rand() & 1)
            {   /* do multiple flip if odd number */
                if (biglist[ires].n)
                {
...
int fitit()
{
    ...
    for (i=0; i<Nx; i++)
    {
        ...
        else if (env.titr_type == 'e')  xp[i] =  env.titr_eh0 + i*env.titr_ehd;
        else  xp[i] = env.titr_ex0 + i*env.titr_exd;   //Amended for extra
    }
   ...
   // load the pairwise interaction again, round the ele and vdw to keep consistent with mfe.py
	...
	//8.617342E-2  = Boltzmann_cst * keV/K (10^3 eV/K)
        if (env.titr_type == 'p' || env.titr_type == 'x') n = fabs(stat.a * 8.617342E-2 * env.monte_temp / 58.0);  /* 2/23/15: same for chem titr  */
        else n = fabs(stat.a * 8.617342E-2 * env.monte_temp);   /* Assume e- titr: (env.titr_type == 'e')  */
...
float fround3(float x)
{
    ...
    sprintf(sbuff, "%.3f", x);
    if (strcmp(sbuff, "-0.000") == 0) strcpy(sbuff, "0.000"); //Added by catalys 2014/4
    y=atof(sbuff);
    return y;
}//float fround3(float x)
...
int print_mfe(int i_res, float mfeP, FILE *pK_fp, FILE *res_fp)
{
    ...
    // if (cut_off < 0.0000) cut_off = 0.5;    // xuyu commented it, 2011/8
    ...
    if (abs(i_high-i_low) == 0) { // do mfe at only one point
        ...
        //@@@ Amended for extra:
        if ( (env.titr_type == 'p')||(env.titr_type == 'x') ) {
        ...
        for (j=0; j<n_all; j++)
        {
            ...
            if ( (env.titr_type == 'p')||(env.titr_type == 'x') ) { // @@@ x: Amended for extra:
        ...
        if ( (env.titr_type == 'p')||(env.titr_type == 'x') ) { // @@@ Amended for extra:
            fprintf(pK_fp, "  %8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%8.2f%10.2f\n",
                   mfe_res[i_res].mfe.vdw0/PH2KCAL,
                   mfe_res[i_res].mfe.vdw1/PH2KCAL,
                   mfe_res[i_res].mfe.tors/PH2KCAL,
...
int get_mfe(int i_res, int t_point)
{
    ...
    {
       ex = env.titr_ex0 + t_point*env.titr_exd;  //@@@ Amended for 'extra energy' titration
    }

    for (j=0; j<mfe_res[i_res].n; j++)
    {
        if (!(strncmp(conflist.conf[mfe_res[i_res].conf[j]].uniqID+3, "DM", 2)))
        {  // dummy conformer
