...
//............... Amended by Cat Chenal to allow for chemical (i.e. 'energy added to DM') titration 5/2014...//
...
int monte2()
{
    ...
    int i_titra;
    float ph, eh, extra, titr_pnt, temp, free_eng; // @@@ extra: for 'added energy' titration
    float rmsd;
    ...
    for (i_titra=0; i_titra<env.titr_steps; i_titra++)
    {
        ...
        ph = env.titr_ph0;
        eh = env.titr_eh0;
        extra = env.titr_ex0; // @@@ For 'added energy' titration

        if (env.titr_type == 'p') {
            ph = env.titr_ph0 + ((float)i_titra)*env.titr_phd;
        }
        else if (env.titr_type == 'e') {
            eh = env.titr_eh0 + ((float)i_titra)*env.titr_ehd;
        }
        else { extra = env.titr_ex0 + ((float)i_titra)*env.titr_exd; }  // @@@ For 'added energy'titration

        fp = fopen(MC_OUT,"a"); fprintf(fp,"\npH = %6.2f  Eh = %6.2f  extra = %6.2f\n", ph, eh, extra); fclose(fp);

        for (ic=0; ic<prot.nc; ic++)
        {
            prot.conf[ic]->E_ph = env.monte_temp/ROOMT * prot.conf[ic]->H * (ph-prot.conf[ic]->pKa) * PH2KCAL;
            prot.conf[ic]->E_eh = env.monte_temp/ROOMT * prot.conf[ic]->e * (eh-prot.conf[ic]->Em) * PH2KCAL/59.0;  // 58.0: at 293K not 298K
            ...
            // "extra" from 'extra energy' titration added to all DM conformers => H2O, ions, ligands
            if (!strncmp(prot.conf[ic]->uniqID+3, "DM", 2)) {  prot.conf[ic]->E_self += extra; } // @@@ For 'added energy'titration
        }
        ...
        while(1)
        {
            ...
            while (1)
            {
                ...
                {
                    if (n_cycle_max != -1) { if (i_cycle >= n_cycle_max) break; }
...
/*
            ...
            else if (env.titr_type == 'e') { titr_pnt = eh; }
            else { titr_pnt = extra; }
...
/* 2014-09-04 >>>>>>>>>>>>>>>>>>> code below from erengies3.c with STEP3_OUT changed to ENERGY_DIR (added in mcce.h) */
    ...
    for (ic=0; ic<prot.nc; ic++) {
        ...
        /* get opp file name */
        if (!env.monte_old_input) {
            sprintf(fname, "%s/%s.opp", ENERGY_DIR, prot.conf[ic]->uniqID);  /* Changed STEP3_OUT to ENERGY_DIR */
        }
        else {
...
double free_unf(PROT prot) {
    ...
    .   G(unfolded)= -kT Sum(i)(# of ionizable) ln(1 + exp(-2.3gamma(i)(pH-pKsol(i))))
    .
    .   An-Suei Yang and Barry Honig, J.Mol.Biol.(1993), 231, 459-474 
    .
    . Expand this equation so it applies to residues with multiple protonation/electron states
    .   G(unfolded) = -kT * Sum(i_res){ ln( Sum(k_state) { exp(-2.3*gamma_proton( i_res,k_state)*(pH-pKa(i_res)) 
    .                                                                             -(2.3/58.)*gamma_electron(i_res,k_state)*(Eh-Em(i_res))) }
    */
...
int monte_out(PROT prot, int n_titra) {
    ...
    {
        fprintf(fp, " extra        ");
        for (i=0; i<n_titra; i++) fprintf(fp, " %5.0f", env.titr_ex0+i*env.titr_exd); // @@@ Amended for 'extra energy' titration
    }
    fprintf(fp, "\n");
    for (ic=0; ic<prot.nc; ic++)
    {
    ...
    {
        fprintf(fp, " extra    ");
        for (i=0; i<n_titra; i++) fprintf(fp, " %5.1f", env.titr_ex0+i*env.titr_exd); // @@@ Amended for 'extra energy' titration
    }
    fprintf(fp, "\n");
    for (i_res=0;i_res<prot.n_res;i_res++)
    {
...
{   int i, j, kc;
    ...
    /* scan conformers to see if all pw were calculated, Debuggubg
...
return 2;
	for(i=0;i<s->num_of_states;i++)
	{
		...
		{                                                               //and protons if it does so this state already exist
			s->IS[i].Nconformers++;     // so here we increment the number of confermer for this state
...
void load_states(RES *r)// to load the ionization states
{
   ...
   for(i=1;i< r->n_conf;i++)//loop over all the confermer of this residue
   {
	...
	s.Nconformers++;//change the number of states for this confermer to one
