...
int rotamers()
{
   ...
   */
   ...
   /* Relabel rotamer history
     Disabled in mcce2.4 version to avoid a conformer explosion in the relaxation
   ...
   if (env.n_hv_conf_limit > 0)
   {
       ...
       if (env.rot_mhd_prune == 1)
       { //execution path that allows for MHD's pruning function
...
{  int i, i_conf;
   ...
   for (i=0; i<prot.n_res; i++) {
      ...
      while (1) {
         ...
         sprintf(rule.affected, sbuff);//ERROR:740:10: warning: format not a string literal and no format arguments [-Wformat-security]
...
{  int i, i_conf;
   ...
   for (i=0; i<prot.n_res; i++) {
      ...
      while (1) {
         ...
         if (param_get("ROTAMER", prot.res[i].resName, C_str, &rule)) break;
         sprintf(sbuff, " %s  ", rule.affected);
         sprintf(rule.affected, sbuff); //ERROR: 973:10: warning: format not a string literal and no format arguments [-Wformat-security]
        ...
        for (j_res=i_res+1; j_res<prot.n_res; j_res++)
        {
            ...
            for (i_conf=1; i_conf<prot.res[i_res].n_conf; i_conf++)
            {
                ...
                for (j_conf=1; j_conf<prot.res[j_res].n_conf; j_conf++)
                {
                    ...
                    /* H bond energy correction: Turned off */
                    /* -> low energy from h bond traps conformer in certain positions, and loses occupancy for other (exposed) conformers; Yifan */
...
int get_demetri_out(void)
{ char line[MAXCHAR_LINE], num[2]=" ", atomname[6], conformerlabel;
  int i;
  FILE *fp, *fp2;
...
/* Special Cases */
         ...
         /*deal with the conformer number part in the next*/
         for(i=23; ' '==line[i]; i++)
            line[i]='0';
         num[1] = line[29];
         num[0] = line[28];
         conformerlabel = atoi(num);
         if(0 == conformerlabel)
            conformerlabel = 65;
         else if(conformerlabel > 0 && conformerlabel <= 26)
            conformerlabel += 64;
         else if(conformerlabel >26 && conformerlabel <=52)
            conformerlabel += 70;
         else
         { printf("\n   The number of conformers exceeds 52!\n");
            return USERERR;
         }
         line[26] = conformerlabel;
         line[29] = ' ';
         line[28] = ' ';
         line[27] = ' ';
         ...
         fprintf(fp2, "%s", line); //corrected:4176:10: warning: format not a string literal and no format arguments [-Wformat-security]
...
int prune_hdirected(PROT prot)
{
   ...
   float vdw_pair;
   float dd, dd_low, dd_high;
   float d_relax; //"warning: d_relax - set but not used"; yet holds output of relax_this_pair()
   char  HaveON;
   char  need_relax;
   ...
   for (ires=0; ires<prot.n_res; ires++)
   {
      ...
      if (!HaveON) continue;  /* next residue when no O or N atoms*/
...
{  int i, n_a, n_b, kc;
   ...
   while (1) {
      ...
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff); //ERROR:4364:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_a->n_conf;
   ...
   while (1) {
      ...
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:4381:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_b->n_conf;
      for (i=n_b; i<kc; i++) {
   ...
   while (1)
   {
      ...
      if (param_get("ROTAMER", res_a->resName, C_str, &rule)) break;
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:4454:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_a->n_conf;
      for (i=n_a; i<kc; i++)
      {
   ...
   while (1)
   {
      ...
      if (param_get("ROTAMER", res_b->resName, C_str, &rule)) break;
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:4471:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_b->n_conf;
      for (i=n_b; i<kc; i++)
      {
   ...
   while (1)
   {
      ...
      if (param_get("ROTAMER", res_a->resName, C_str, &rule)) break;
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:4538:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_a->n_conf;
      for (i=n_a; i<kc; i++)
      {
   ...
   while (1)
   {
      ...
      if (param_get("ROTAMER", res_b->resName, C_str, &rule)) break;
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:4555:7: warning: format not a string literal and no format arguments [-Wformat-security]
      kc = res_b->n_conf;
      for (i=n_b; i<kc; i++)
      {
...
{
  int kr, kc;
  //int i; for 5th percentile analysis
  float max_sas;
...
{  int nconf,i, kc, ka;
   ...
   while (1) {
      ...
      sprintf(sbuff, " %s  ", rule.affected);
      sprintf(rule.affected, sbuff);//ERROR:5056:7: warning: format not a string literal and no format arguments [-Wformat-security]
