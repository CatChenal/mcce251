...
float    **pairwise, **pairwise_vdw;
float    E_base, E_state;
float    ph, eh, extra; // extra for ex tritration
int      *state, *state_bak;
...
int monte()
{  int i, j, k, counter, kc;  // kc for extra titration
    ...
    for (i=0; i<env.titr_steps; i++) {
        ...
        else  if (env.titr_type == 'e') eh = env.titr_eh0 + i*env.titr_phd;
        else extra = env.titr_ex0 + i*env.titr_exd;  // this is for extra titration
    ...
    if (env.titr_type == 'p') {
        ...
        fprintf(fp, " extra        ");
        for (i=0; i<env.titr_steps; i++) fprintf(fp, " %5.0f", env.titr_ex0+i*env.titr_exd);
    }
...
// "extra" from Extra titration added to all non-dummy conformers?
...
{  int i, j;
    ...
    for (i=0; i<Nx; i++) {
        ...
        else  xp[i] = env.titr_ex0 + i*env.titr_exd;
    }
    ...
    Counter = 0;
    for (i=0; i <conflist.n_conf; i++) {
        //if (strchr(conflist.conf[i].uniqID, '+') || strchr(conflist.conf[i].uniqID, '-')|| !strncmp(conflist.conf[i].uniqID+3, "DM", 2)) {
        if (!strncmp(conflist.conf[i].uniqID+3, "DM", 2)) {
   ...
   // for (i=0; i<Nx; i++) fprintf(fp, " %5d", (int) xp[i]);
    for (i=0; i<Nx; i++) fprintf(fp, " %5.1f", xp[i]);
    fprintf(fp, "\n");
    for(i=0; i<N_res; i++) {
    ...
    else if (env.titr_type == 'e') {   /* eH titration */
        fprintf(fp, "  Eh      ");
    }
    else       // extra titration assumed 
        fprintf(fp, "  Extra   ");
