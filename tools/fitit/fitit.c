...
{   FILE *fp, *fp2;
    ...
    /*<<< Get envelope to get titr type */
    printf("   Load run control file \"%s\"...\n", FN_RUNPRM); fflush(stdout);
    // Call to get_enc: collects all the parameters in struc
...
//        printf("File sum_crg.out not found\n");  /* why not fort.38??? */
    if (!(fp = fopen("fort.38.non0", "r"))) {
        printf("File fort.38.non0 not found\n");
        return 0;
    }
    else {
...
*/
       ...
       Nx = (strlen(sbuff)-11)/6;  /* -11 = for 1st col + 1 space */
       ...
       fclose(fp);
//       N_res -= 4; /* extra lines in sum_crg.out*/
...
/**/
    ...
    else if (env.titr_type == 'c') {   /* chem titration */
        printf("  ch      ");
    }
    else {      /* Eh titration */
        printf("  Eh      ");
    }
    printf("       pK/Kd/Em  n  1k*chi2\n");
...
/*        if (mid >= 0.485) {
            if (fabs(yp[0]-yp[Nx-1])>0.5) {  // jumps form <0.015 to >0.985
               printf("%s          titration curve too sharp\n", shead[i]);
            }
...
*/
        ...
        if (env.titr_type == 'p' || env.titr_type == 'c') n = fabs(stat.a * 8.617342E-2 * 273.15 / 58.0);
