...
int Monte()
{
    ...
    for (i_titra=0;i_titra<prot.param.titra_steps;i_titra++) {
        ...
        while(1) {
            ...
            if (i_red >=3) Monte_set_toggle(prot_w);
            ...
            for (i_cycle=0;i_cycle<n_cycle;i_cycle++) {
                ...
                if (i_red ==0) Monte_update_energy(prot_red); /* update TS correction in the first reduce run, later it is inherited from prot_w */
            }
            ...
            while (1) {
                ...
                */
                for (ic=0;ic<prot_red.sidechain.size();ic++) {
                    ...
                    prot_red.sidechain[ic]->occ_table.push_back(occ_of_this_cycle);
                }
                
                if (i_red ==0) {
                    Monte_update_energy(prot_red);
...
void Monte_MC(Prot& prot)
{
    ...
    while (i_iter) {
        ...
        /* select one residue */
        k_weighted = (int) (ran2(&dummy_num) * (double)prot.weighted_res_list.size());
        k_res = prot.weighted_res_list[k_weighted];
        //k_res = (int) (ran2(&dummy_num) * (double)prot.res.size());

        
        /* decide how many flips */
        n_flip = (int) (ran2(&dummy_num) * (double)prot.res[k_res].n_flip_max) + 1;
        ...
        for (i_flip=1;i_flip<n_flip;i_flip++) {
            ...
            k_ngh = (int) (ran2(&dummy_num) * (double)n_ngh);
        ...
        for (i_flip=0;i_flip<n_flip;i_flip++) {
            ...
            k_conf = (int)(ran2(&dummy_num) * (double)(flip_res[i_flip]->conf.size()-1)) + 1;
