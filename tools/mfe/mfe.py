...
scale_dsolv = 1.0
fname_extra = ""
ph2Kcal = 1.364   # at 25 Celcius only
mev2Kcal = 0.0235
Kcal2kT  = 1.688
conformers = []
residues = []
...
def read_fort38():
   ...
   if   ttype == 'EH':
      ...
      titration_type = 'ch'
      titration_unit = 'kcal/mol'	#pH'   # should it not be kcal/mol?
   else:
   ...
   for i in range(len(conformers)):
      ...
      if conformers[i].id != line[0]:
         ...
         return
      conformers[i].occ = [float(x) for x in line[1:]]   # conformer occ from fort.38
   return
...
##		print "DEBUG::line[3]: %s ::len: %6.4: " % (line[3], len(line[3]))
           	while len(line[3]) > 8:
     ...
     # make conformer mfe
      for i in range(len(titration_range)):
        ...
        if titration_type == 'pH': # or titration_type == 'ch':
...
if __name__ == '__main__':
   ...
   if not t_low_found or not t_high_found:
      print "titration_point out of range"
      sys.exit(0)
...
##   print sys.argv[1];
   ...
   print "Residue     	%s   		%s:pK" % (sys.argv[1], res_pKa)
